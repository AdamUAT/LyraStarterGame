...
#include "Character/LyraGravityCharacterMovementComp.h"
...
ULyraGravityCharacterMovementComp::ULyraGravityCharacterMovementComp(const FObjectInitializer& ObjectInitializer)
...
FVector ULyraGravityCharacterMovementComp::GetGravityDirection(bool bAvoidZeroGravity) const
{
...
void ULyraGravityCharacterMovementComp::SetGravityDirection(FVector NewGravityDirection)
{
	CustomGravityDirection = NewGravityDirection.GetSafeNormal();
}

void ULyraGravityCharacterMovementComp::PhysFlying(float deltaTime, int32 Iterations)
{
...
float ULyraGravityCharacterMovementComp::BoostAirControl(float DeltaTime, float TickAirControl, const FVector& FallAcceleration)
{
...
void ULyraGravityCharacterMovementComp::OnMovementModeChanged(EMovementMode PreviousMovementMode, uint8 PreviousCustomMode)
{
	if (!HasValidData())
	{
		return;
...
void ULyraGravityCharacterMovementComp::PerformMovement(float DeltaTime)
{
	if (!HasValidData())
	{
		return;
...
void ULyraGravityCharacterMovementComp::HandleImpact(const FHitResult& Hit, float TimeSlice /*= 0.f*/, const FVector& MoveDelta /*= FVector::ZeroVector*/)
{
	if (CharacterOwner)
	{
...
void ULyraGravityCharacterMovementComp::ProcessLanded(const FHitResult& Hit, float remainingTime, int32 Iterations)
{
...
void ULyraGravityCharacterMovementComp::MaintainHorizontalGroundVelocity()
{
	return;
...
float ULyraGravityCharacterMovementComp::SlideAlongSurface(const FVector& Delta, float Time, const FVector& Normal, FHitResult& Hit, bool bHandleImpact)
{
	if (!Hit.bBlockingHit)
	{
		return 0.0f;
...
void ULyraGravityCharacterMovementComp::SetPostLandedPhysics(const FHitResult& Hit)
{
	if (CharacterOwner)
	{
...
void ULyraGravityCharacterMovementComp::PhysWalking(float deltaTime, int32 Iterations)
{
...
FVector ULyraGravityCharacterMovementComp::ComputeGroundMovementDelta(const FVector& Delta, const FHitResult& RampHit, const bool bHitFromLineTrace) const	// OK
{
...
void ULyraGravityCharacterMovementComp::MoveAlongFloor(const FVector& InVelocity, float DeltaSeconds, FStepDownResult* OutStepDownResult /*= NULL*/)
{
	if (!CurrentFloor.IsWalkableFloor())
	{
		return;
...
void ULyraGravityCharacterMovementComp::SimulateMovement(float DeltaTime)
{
...
FVector ULyraGravityCharacterMovementComp::ConstrainInputAcceleration(const FVector& InputAcceleration) const
{
...
FVector ULyraGravityCharacterMovementComp::ScaleInputAcceleration(const FVector& InputAcceleration) const
{
	return GetMaxAcceleration() * GetClampedToMaxSizePrecise(InputAcceleration, 1.0f);
}

FVector ULyraGravityCharacterMovementComp::CalcAnimRootMotionVelocity(const FVector& RootMotionDeltaMove, float DeltaTime, const FVector& CurrentVelocity) const
{
...
void ULyraGravityCharacterMovementComp::StartFalling(int32 Iterations, float remainingTime, float timeTick, const FVector& Delta, const FVector& subLoc)
{
	const float DesiredDist = Delta.Size();
...
void ULyraGravityCharacterMovementComp::PhysFalling(float deltaTime, int32 Iterations)
{
	if (deltaTime < MIN_TICK_TIME)
	{
		return;
...
FVector ULyraGravityCharacterMovementComp::GetFallingLateralAcceleration(float DeltaTime)
{
	const FVector GravityDirection = GetGravityDirection();
...
FVector ULyraGravityCharacterMovementComp::NewFallVelocity(const FVector& InitialVelocity, const FVector& Gravity, float DeltaTime) const
{
...
void ULyraGravityCharacterMovementComp::UpdateBasedMovement(float DeltaSeconds)
{
	if (!HasValidData())
	{
		return;
...
bool ULyraGravityCharacterMovementComp::DoJump(bool bReplayingMoves)
{
...
FVector ULyraGravityCharacterMovementComp::GetImpartedMovementBaseVelocity() const
{
...
void ULyraGravityCharacterMovementComp::JumpOff(AActor* MovementBaseActor)
{
...
void ULyraGravityCharacterMovementComp::FindFloor(const FVector& CapsuleLocation, FFindFloorResult& OutFloorResult, bool bZeroDelta, const FHitResult* DownwardSweepResult /*= NULL*/) const
{
	...
	{
		ULyraGravityCharacterMovementComp* MutableThis = const_cast<ULyraGravityCharacterMovementComp*>(this);
...
void ULyraGravityCharacterMovementComp::UpdateBasedRotation(FRotator& FinalRotation, const FRotator& ReducedRotation)
{
...
bool ULyraGravityCharacterMovementComp::FloorSweepTest(struct FHitResult& OutHit, const FVector& Start, const FVector& End, ECollisionChannel TraceChannel,
...
bool ULyraGravityCharacterMovementComp::IsValidLandingSpot(const FVector& CapsuleLocation, const FHitResult& Hit) const
{
	if (!Hit.bBlockingHit)
	{
		return false;
...
bool ULyraGravityCharacterMovementComp::ShouldCheckForValidLandingSpot(float DeltaTime, const FVector& Delta, const FHitResult& Hit) const
{
	const FVector CapsuleUp = GetCapsuleAxisZ();
...
bool ULyraGravityCharacterMovementComp::ShouldComputePerchResult(const FHitResult& InHit, bool bCheckRadius /*= true*/) const
{
...
bool ULyraGravityCharacterMovementComp::ComputePerchResult(const float TestRadius, const FHitResult& InHit, const float InMaxFloorDist, FFindFloorResult& OutPerchFloorResult) const
{
...
bool ULyraGravityCharacterMovementComp::CanStepUp(const FHitResult& Hit) const
{
...
bool ULyraGravityCharacterMovementComp::StepUp(const FVector& GravDir, const FVector& Delta, const FHitResult& Hit, FStepDownResult* OutStepDownResult /*= NULL*/)
{
...
void ULyraGravityCharacterMovementComp::SetMovementMode(EMovementMode NewMovementMode, uint8 NewCustomMode /*= 0*/)
{
...
void ULyraGravityCharacterMovementComp::SetDefaultMovementMode()
{
...
void ULyraGravityCharacterMovementComp::AdjustFloorHeight()
{
...
bool ULyraGravityCharacterMovementComp::CheckLedgeDirection(const FVector& OldLocation, const FVector& SideStep, const FVector& GravDir) const
{
...
FVector ULyraGravityCharacterMovementComp::GetLedgeMove(const FVector& OldLocation, const FVector& Delta, const FVector& GravDir) const
{
...
void ULyraGravityCharacterMovementComp::StartNewPhysics(float deltaTime, int32 Iterations)
{
...
void ULyraGravityCharacterMovementComp::CalcVelocity(float DeltaTime, float Friction, bool bFluid, float BrakingDeceleration)
{
...
void ULyraGravityCharacterMovementComp::ApplyAccumulatedForces(float DeltaSeconds)
{
...
bool ULyraGravityCharacterMovementComp::IsWalkable(const FHitResult& Hit) const
{
	if (!Hit.IsValidBlockingHit())
	{
...
void ULyraGravityCharacterMovementComp::ComputeFloorDist(const FVector& CapsuleLocation, float LineDistance, float SweepDistance, FFindFloorResult& OutFloorResult, float SweepRadius, const FHitResult* DownwardSweepResult /*= NULL*/) const
{
	OutFloorResult.Clear();
...
bool ULyraGravityCharacterMovementComp::IsWithinEdgeTolerance(const FVector& CapsuleLocation, const FVector& TestImpactPoint, const float CapsuleRadius) const
{
	const FVector CapsuleDown = GetCapsuleAxisZ() * -1.0f;
...
FVector ULyraGravityCharacterMovementComp::GetGravity() const
{
	if (!CustomGravityDirection.IsZero())
	{
...
FVector ULyraGravityCharacterMovementComp::GetComponentDesiredAxisZ() const
{
	return GetGravityDirection(true) * -1.0f;
}

void ULyraGravityCharacterMovementComp::UpdateComponentRotation()
{
	if (!UpdatedComponent)
	{
		return;
...
inline FQuat ULyraGravityCharacterMovementComp::GetCapsuleRotation() const
{
	return UpdatedComponent->GetComponentQuat();
}

inline FVector ULyraGravityCharacterMovementComp::GetCapsuleAxisX() const
{
...
inline FVector ULyraGravityCharacterMovementComp::GetCapsuleAxisZ() const
{
...
// Version that does not use inverse sqrt estimate, for higher precision.
inline FVector ULyraGravityCharacterMovementComp::GetSafeNormalPrecise(const FVector& V)
{
	const float VSq = V.SizeSquared();
	if (VSq < SMALL_NUMBER)
	{
...
bool ULyraGravityCharacterMovementComp::IsWithinEdgeToleranceNew(const FVector& CapsuleLocation, const FVector& CapsuleDown, const FVector& TestImpactPoint, const float CapsuleRadius) const
{
	const float DistFromCenterSq = (CapsuleLocation + CapsuleDown * ((TestImpactPoint - CapsuleLocation) | CapsuleDown) - TestImpactPoint).SizeSquared();
	const float ReducedRadiusSq = FMath::Square(FMath::Max(KINDA_SMALL_NUMBER, CapsuleRadius - SWEEP_EDGE_REJECT_DISTANCE));
